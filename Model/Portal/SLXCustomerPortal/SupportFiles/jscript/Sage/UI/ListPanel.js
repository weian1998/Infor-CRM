...
function (aspect,
    ...
    var listPanel = declare('Sage.UI.ListPanel', [borderContainer, _Widget, _Templated], {
        ...
        _setSortInfo: function (store) {
            ...
            else if (store.id == "Entity" || store.id == "all_groups" || store.id == "releases" || store.id == "statistics") {
                if(this.hasCheckBox){
                    this._listGrid.sortInfo = 2;
                }
                else{
                    this._listGrid.sortInfo = 1;
                }
            }
            else {
        ...
        *
        ...
        _doAutoFitColumns: function () {
            ...
            var storeId = this._listGrid.store.id;
            if (storeId == "all_groups" || storeId == "releases" || storeId == "statistics") {
            ...
            array.forEach(cells, function (item) {
                ...
                if (item && item.field && !this._isFieldHidden(item.field)) {
                    ...
                    newWidth = Math.floor(realWidth * pct);
        ...
        _updateStructureFromLocalStore: function () {
            ...
            if (fields) {
                if (structure[0].cells) {
                    ...
                    //sdata list views don't contain cells object, and rather than changing
                    //each one of them we'll just use the structure.
                    cells = structure;
                }
                if (cells) {
        ...
        _setDetailPaneVisibility: function (value, saveState) {
            if (value) {
                ...
                if (selected) {
                    ...
                    this._detailPane.clear();
                    if (this._configurationProvider && this.configurationProvider._mainViewName === "EntityManager") {
                        aspect.after(this._listGrid, "_onFetchComplete", function () {
                            dojo.publish('/sage/ui/grid/gridLoadComplete', '');
                        });
                    }
                }
