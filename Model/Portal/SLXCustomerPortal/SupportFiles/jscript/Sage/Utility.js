    ...
    dojo.mixin(Sage.Utility, {
        ...
        */
        ...
        openPdfDoc: function (topic) {
            ...
            window.open(url + helpPath + topic);
        },
        ...
        buildOwnersCache: function (count, start) {
            ...
            start = start || 1;
            return xhr('GET', {
                url: dString.substitute("slxdata.ashx/slx/dynamic/-/owners?select=Type&orderBy=$key&format=json&count=${0}&startIndex=${1}", [count, start]),
                handleAs: "json",
        ...
        buildUserCache: function (count, start) {
            ...
            start = start || 1;
            return xhr('GET', {
                url: dString.substitute("slxdata.ashx/slx/dynamic/-/users?select=Enabled&format=json&Count=${0}&StartIndex=${1}", [count, start]),
                handleAs: "json",
                sync: true  // we have to do a sync call here, because the grid column needs to get HTML.
                // but, most of the time this will be cached, so it should not be a big impact.
            }).then(function (data) {
