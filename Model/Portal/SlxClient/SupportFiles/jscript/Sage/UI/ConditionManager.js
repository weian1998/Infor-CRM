        ...
        getConditionsAsUrlWhereString : function () {
            ...
            for (i = 0; i < conds.length; i++) {
                ...
                if (typeof condVal === 'string') {
                    if (conds[i].operator === 'like' || conds[i].operator === 'sw') {
                        // these characters need to be used as a value and not as an operator 
                        var reSymbols = new RegExp("[%]|\\[|[*]|[_]", "g");
                        condVal = condVal.replace(reSymbols, "[$&]");
                    }

                    // double up quotations to allow their value to be searched for 
                    //  - instead of being used as a delimiter for the search
                    var reQuotes = new RegExp("[\"]|[']", "g");
                    condVal = condVal.replace(reQuotes, "$&$&");
                }
