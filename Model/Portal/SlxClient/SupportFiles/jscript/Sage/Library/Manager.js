...
// ReSharper disable InconsistentNaming
    function (SDataServiceRegistry, FileHandler, FolderHandler, RoleSecurityService, SystemOptions,
        Dialogs, Utility, DataGrid, Button, Toolbar, Tree, ForestStoreModel, ItemFileWriteStore, i18n, dString,
        ...
        dojo.mixin(Sage.Library.Manager, {
            ...
            refreshGrid: function (item, start, count) {
                var self = this;
                ...
                if (start == 1 && item)// if a folder was selected and on the first page.
                {
                    ...
                    this._grid.setStore(new ItemFileWriteStore({ data: { items: [] } }));
                }
                ...
                oRequest.read({
                    success: function (feed) {
                        ...
                        try {
                            ...
                            for (var i = 0; i <= files.items.length - 1; i++) {
                                ...
                                if (files.items[i].$key) {
                                    ...
                                    self._grid.store.newItem(files.items[i]);
                                }
                            }
                        ...
                        // if we need to go back and get more files
                        if (totalResults > defaultPageSize && totalResults >= (start + count)) {
                            start = start + count;
                            self.refreshGrid(item, start, count);
                        }
                    },
                    failure: function (xhr, sdata) {
                        self._hideLoading();
