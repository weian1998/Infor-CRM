...
public class OAuthBase
{
    ...
    /// <param name="additionalParams">Any additional parameter to be included in the signature.  They will also be returned as part of normalizedRequestParameters.  
    /// The values must be Url-encoded using the OAuthBase.UrlEncode method (NOT HTTPUtility.UrlEncode)</param>
    ...
    public string GenerateSignatureBase(Uri url, string consumerKey, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, string signatureType,
        ...
        params QueryParameter[] additionalParams)
    {
        if (token == null)
        {
            token = string.Empty;
        ...
        params QueryParameter[] additionalParams)
    {
        return GenerateSignature(url, consumerKey, consumerSecret, token, tokenSecret, httpMethod, timeStamp, nonce, SignatureTypes.HMACSHA1,
            out normalizedUrl, out normalizedRequestParameters, additionalParams);
    }
    ...
    public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType,
        out string normalizedUrl, out string normalizedRequestParameters, params QueryParameter[] additionalParams)
    {
        normalizedUrl = null;
        normalizedRequestParameters = null;
        ...
        switch (signatureType)
        {
            ...
            case SignatureTypes.HMACSHA1:
                string signatureBase = GenerateSignatureBase(url, consumerKey, token, tokenSecret, httpMethod, timeStamp, nonce, HMACSHA1SignatureType, out normalizedUrl, out normalizedRequestParameters, additionalParams);
